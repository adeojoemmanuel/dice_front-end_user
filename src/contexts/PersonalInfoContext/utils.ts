...
const searchNFTAsArray = (nft_symbol: any) => {
  let flag = false;
  nfts.accounts.forEach((element: any) => {
    if (nft_symbol.includes(element.name)) {
      flag = true;
    }
  });
  return flag;
...
const searchNFTasObject = (nft_symbol: any) => {
  let flag = false;
  nfts.accounts.forEach((element: any) => {
    if (nft_symbol.includes(element.name)) {
      flag = true;
...
export async function getTokensForOwner(owner: any) {
  ...
  for (let i = 0; i < nftinfo.length; i++) {
    ...
    // }
    const temp = searchNFTasObject(metadata.data.data.symbol);
    if (temp) {
